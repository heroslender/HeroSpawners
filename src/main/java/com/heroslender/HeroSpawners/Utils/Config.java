...
public class Config {
    ...
    private static void verificarConfig() {
        ...
        if (!config.contains("spawner.holograma"))
            config.set("spawner.holograma", TEXTO_HOLOGRAMA);
...
//                entityNames.put(e, config.getString("mobs." + e.name(), e.getName()));
    ...
    private static void loadConfig() {
        FileConfiguration config = HeroSpawners.getInstance().getConfig();
        TEXTO_HOLOGRAMA = ChatColor.translateAlternateColorCodes('&', config.getString("spawner.holograma", TEXTO_HOLOGRAMA));
